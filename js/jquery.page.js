...
(function($){
	...
	function Page(container,fn,args,style){
		...
		var args=this.args=$.extend({
			...
			align:'center'//对齐方式。默认右对齐.可选：left,right
		},args||{});
	...
	Page.prototype.fillHtml=function(){
		...
		if(args.showNext){
			if(args.current== args.pageCount||args.pageCount==1){
				obj.remove('.nextPage');
				obj.append('<span class="disabled xia">下一页</span>');
			}else{
	...
	Page.prototype.setStyle=function(){
		...
		// this.obj.find('a.prevPage').css({'width':s.prevNextWidth,'font-size':s.fontSize+'px','height':s.height-2+'px','line-height':s.height-2+'px','color':'#000'});
		// this.obj.find('a.nextPage').css({'width':s.prevNextWidth,'font-size':s.fontSize+'px','height':s.height-2+'px','line-height':s.height-2+'px','color':'#000'});
		...
		// this.obj.find('span.disabled').css({'display':'inline-block','height':s.height-2+'px','line-height':s.height-2+'px','color':s.disableColor,'border':'1px solid '+s.borderColor,'vertical-align':'middle','font-size':s.fontSize+'px','width':s.prevNextWidth+'px'});
		...
		this.obj.find('.tcdNumber').css({'width':s.pageWidth-2+'px','height':s.height-2+'px','line-height':s.height-2+'px','color':'#8F8F8F'});
